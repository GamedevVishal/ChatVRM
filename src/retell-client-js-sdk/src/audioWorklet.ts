...
class captureAndPlaybackProcessor extends AudioWorkletProcessor {
    ...
    process(inputs, outputs, parameters) {
      ...
      // Calculate volume on output for both channels
      let sum1 = 0;
      for (let i = 0; i < outputChannel1.length; i++) {
        sum1 += outputChannel1[i] * outputChannel1[i];
      }
      let rms1 = Math.sqrt(sum1 / outputChannel1.length);
      let volume1 = Math.max(rms1, 0.01); // avoid volume being 0
      volume1 = Math.round(volume1 * 100)
